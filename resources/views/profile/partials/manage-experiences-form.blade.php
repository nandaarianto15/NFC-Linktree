    ...
    <div class="p-4 bg-slate-50/50 dark:bg-white/[0.02] rounded-xl border border-slate-200/50 dark:border-white/[0.05] mb-6 transition-colors duration-500">
        <form action="{{ route('experiences.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wider transition-colors duration-500">Posisi / Role</label>
    ...
    @else
        <div class="space-y-3">
            @foreach($user->experiences as $exp)
                <div class="group relative flex items-start gap-4 p-4 bg-white/50 dark:bg-white/[0.02] rounded-xl border border-slate-200/50 dark:border-white/[0.05] hover:border-sky-200/60 dark:hover:border-sky-500/20 transition-all duration-200">
                    {{-- Timeline dot --}}
                    ...
                    {{-- Delete --}}
                    <form action="{{ route('experiences.destroy', $exp->id) }}" method="POST" onsubmit="return confirm('Hapus pengalaman ini?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="opacity-0 group-hover:opacity-100 p-1.5 text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 rounded-lg transition-all duration-200 cursor-pointer shrink-0" title="Hapus">
